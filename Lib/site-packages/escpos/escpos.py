...
encodings = {
        'cp437': CHARCODE_PC437,
        'cp850': CHARCODE_PC850,
        'cp852': CHARCODE_PC852,
        'cp858': CHARCODE_PC858,
}
...
class Escpos(object):
    ...
    def _encode_string(self, text):
        """
        Encodes a string utf-8 character into a sequence of
        esc-pos code page change instructions and character declarations
        """
        text_utf8 = text.encode('utf-8')
        encoded_text = text_utf8
        self.encoding = 'cp850'
        if self.encoding == 'cp850':
            for latin_char in list(TXT_LATIN_MAP.keys()):
                if latin_char in text_utf8:
                    encoded_text = text_utf8.replace(
                            latin_char,
                            TXT_LATIN_MAP[latin_char],
                        )
        return encoded_text

    def text(self, txt):
        """ Print alpha-numeric text """
        txt = self._encode_string(txt)
        if txt:
            if type(txt) == str:
                txt = bytes(txt, 'UTF-8')
            self._raw(txt)
